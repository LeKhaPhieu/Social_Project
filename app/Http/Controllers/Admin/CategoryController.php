...
class CategoryController extends Controller
{
    ...
    {
        $result = $this->categoryService->deleteCategory($request->name);

        if ($result) {
            return redirect()->route('admin.dashboard')->with('success', 'xóa category thành công');
        }

        return redirect()->route('admin.dashboard')->with('error', 'xóa category thất bại');
    }
}
